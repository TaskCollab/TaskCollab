...
import TasksTable from './TasksTable'; // Ensure this is imported if used
...
const TaskDetails: React.FC = () => {
        ...
        const fetchTask = async () => {
            try {
                ...
                if (data && data.id !== undefined) {
                    ...
                    setIsLocked(formattedTask.locked);
    ...
    }, [taskId, navigate]); // Added navigate to dependency array
    ...
    const handleSelectChange = (e: SelectChangeEvent<string>) => {
        if (editedTask) {
            setEditedTask({
                ...editedTask,
                [e.target.name]: e.target.value as 'High' | 'Medium' | 'Low' | 'Open' | 'In Progress' | 'Completed',
            });
        }
    };
        ...
        <Box sx={{ p: 4, maxWidth: 600, margin: '0 auto' }}>
            ...
            {editMode ? (
                ...
                <Box>
                    ...
                    <Box sx={{ mt: 2 }}>
                        <Button variant="contained" onClick={() => setEditMode(true)} sx={{ mr: 1 }} disabled={isLocked}>
                            Edit
                        </Button>
                        <Button variant="outlined" color="error" onClick={handleDelete} disabled={isLocked}>
                            Delete
                        </Button>
                        {isAdmin && (
            ...
            <Snackbar open={!!error} autoHideDuration={6000} onClose={() => setError(null)} message={error} anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }} />
        </Box>
    );
};
